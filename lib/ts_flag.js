...
var TSFlag = /** @class */ (function () {
    ...
    */
    TSFlag.prototype.str = function (name, initValue, desc) {
        this.setArgObj({ name: name, type: 'string', optionVal: initValue, description: desc });
        var optionIndex = this.getOptionIndex(name);
        var optionVal = this._args[optionIndex].split("=")[1];
        if (!optionVal)
            return initValue;
    ...
    */
    TSFlag.prototype.getOptionIndex = function (name) {
        var optionIndex = this._args.findIndex(function (arg) {
            var option = arg.split("=")[0].trim();
            return option === name || option === "-" + name || option === "--" + name;
        });
        // NOTE - if not contained, throw error
        if (optionIndex < 0)
            throw new ReferenceError(name + " option is not contained");
        return optionIndex;
    };
    return TSFlag;
}());
